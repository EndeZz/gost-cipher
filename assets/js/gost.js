...
class GOST {
  ...
  textToPseudoBinary(text, charSize = CHAR_SIZE) {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      const placeholder = zero.repeat(charSize - pseudoBinary.length);
  ...
  f(A, X) {
    ...
    const ASplit = this.splitPseudoBinary(binarySum, 8); //Разбить на 8 4х-битовых подпоследовательностей
    const ASplitMutated = [];
